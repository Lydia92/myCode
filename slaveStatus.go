...
func main(){
	...
	//res,err:=engine.Where("id = 1").Query()
	if err	!=nil{
		...
		for _,v :=range res {
			...
			for k,vv:=range v{
				...
				case "Relay_Log_File":
					rr["Relay_Log_File"]=result["Relay_Log_File"]
				case "Relay_Log_Pos":
					rr["Relay_Log_Pos"]=result["Relay_Log_Pos"]
	...
	/*	if rr["Master_Log_File"]==rr["Relay_Master_Log_File"]{
		...
		}else if  result["Exec_Master_Log_Pos"]==result["Read_Master_Log_Pos"]{}*/
	if rr["Master_Log_File"]==rr["Relay_Master_Log_File"]{
